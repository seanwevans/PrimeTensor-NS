...
/-!
# Classicalization: time continuity of evaluated third Fréchet derivatives

`CubicInverseFrechetContinuity` proves that, at every strict positive interior
restart time `s`, the evaluated third Fréchet derivative of the complex
representative of the spectral difference state

    W(r)_i - W(s)_i

has norm tending to zero as `r → s`.

To use this in the time derivative candidate for the first spatial derivative
of the selected Duhamel reconstruction, we need the ordinary continuity
statement for the third spatial Fréchet derivative of the selected state
itself.

No new estimate is required.  The existing third-jet layer already proves
exact reconstruction subtraction-linearity
...
identifies the third derivative of the reconstructed difference with the
difference of the two third derivatives.

Consequently

    ‖D³ Rep(W(r)_i)(x)[m] - D³ Rep(W(s)_i)(x)[m]‖ → 0,

which is exactly the metric criterion for `ContinuousAt` of every fixed
third-Fréchet evaluation.

No heat-kernel, endpoint, forcing, or Navier--Stokes estimate is introduced.
-/
...
noncomputable local instance axisFintypeH3SchwartzClassicalizationCubicThirdFrechetTimeContinuity
    (d : Depth) :
    Fintype (PrimeTensor.Axis d) :=
  axisFintypeH3SchwartzFrechetInductionMomentAlgebra d

/-- The selected complex representative's evaluated third spatial Fréchet
derivative has difference norm tending to zero at every strict positive
interior restart time. -/
theorem h3SpectralFinHeatLerayMildSolutionAtRestartRadius_C1Representative_thirdFrechet_eval_sub_norm_tendsto_zero
    {ν A s : ℝ}
    (hν : 0 < ν)
...
/-- Every fixed evaluation of the selected complex representative's third
spatial Fréchet derivative is time-continuous at every strict positive
interior restart time. -/
theorem h3SpectralFinHeatLerayMildSolutionAtRestartRadius_C1Representative_thirdFrechet_eval_continuousAt
    {ν A s : ℝ}
    (hν : 0 < ν)
    (U₀ : H3SpectralVelocityState)
    (hA : 0 < A)
  ...
  have hNormSub :
    ...
    dsimp only [J]
    simpa only [W] using
      h3SpectralFinHeatLerayMildSolutionAtRestartRadius_C1Representative_thirdFrechet_eval_sub_norm_tendsto_zero
        hν U₀ hA hU₀ hs hsR i x m
