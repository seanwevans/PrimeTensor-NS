import PrimeTensor.Fluid.Vorticity.Continuation.Restart.Mild.Sobolev.Schwartz.Spectral.Classicalization.Physical.Tail.Unit.Viscosity.Frontier.Endpoint.Continuity.Third.SelectedOldWeakStrongWeakFTCEnergyUniformIntegralError

/-!
# Exact weak-energy inequality from scalar old weak FTC

The entire mesh argument has now been transported from temporal product-space
integrability to the scalar divergence-free old weak-FTC interface.

This file performs the final bookkeeping:
...
The output is the exact Grönwall-ready relative-energy inequality under only
the scalar old projected-RHS weak FTC.
-/
...
noncomputable local instance axisFintypeH3SelectedOldWeakStrongWeakFTCEnergyExact
    (d : Depth) :
    Fintype (PrimeTensor.Axis d) :=
  Fintype.ofFinite (PrimeTensor.Axis d)

/-- The genuine energy part of the scalar-weak-FTC uniform partition
telescopes exactly to the single interval `[0,q]`. -/
theorem sum_selectedOldDifference_uniformPartition_energyIntegral_eq_of_allProjectedRHSWeakFTC
    {E : ℝ}
...
/-- Finite uniform-mesh weak-energy inequality under only scalar old weak FTC. -/
theorem norm_sq_selectedOldDifferenceReal_le_energyIntegral_add_uniformErrors_of_allProjectedRHSWeakFTC
    {E : ℝ}
    {u : SpaceTimeVectorField ℝ ℝ MulReal Depth.three}
    {T t tau q ε : ℝ}
    (hNS : LoggedPreterminalNavierStokesAdmissible u T)
  ...
  have hPart :=
    norm_sq_selectedOldDifferenceReal_le_uniformPartitionApproximateEnergyIncrement_of_allProjectedRHSWeakFTC
      hNS ht htau hEnd hE hTail htauR hWeakFTC
      hq n hn hε
  ...
  have hEnergy :=
    sum_selectedOldDifference_uniformPartition_energyIntegral_eq_of_allProjectedRHSWeakFTC
      hNS ht htau hEnd hE hTail htauR hWeakFTC
      hq n hn

  have hError :=
    sum_selectedOldDifference_uniformPartition_cellIntegralError_le_of_allProjectedRHSWeakFTC
      hNS ht htau hEnd hE hTail htauR hWeakFTC
      hq n hn hε.le

  have hRemainder :=
    sum_selectedOldDifference_uniformPartition_endpointAndQuadratic_le_of_allProjectedRHSWeakFTC
      hNS ht htau hEnd hE hTail htauR hWeakFTC
      hq n hn hε.le
...
/-- Remove the uniform mesh parameter under scalar old weak FTC. -/
theorem norm_sq_selectedOldDifferenceReal_le_energyIntegral_add_epsilonError_of_allProjectedRHSWeakFTC
    {E : ℝ}
    {u : SpaceTimeVectorField ℝ ℝ MulReal Depth.three}
    {T t tau q ε : ℝ}
    (hNS : LoggedPreterminalNavierStokesAdmissible u T)
  ...
  have hBound :
    ...
    have hFinite :=
      norm_sq_selectedOldDifferenceReal_le_energyIntegral_add_uniformErrors_of_allProjectedRHSWeakFTC
        hNS ht htau hEnd hE hTail htauR hWeakFTC
...
/-- Exact selected--old relative-energy inequality under scalar old weak FTC. -/
theorem norm_sq_selectedOldDifferenceReal_le_energyIntegral_of_allProjectedRHSWeakFTC
    {E : ℝ}
    {u : SpaceTimeVectorField ℝ ℝ MulReal Depth.three}
    {T t tau q : ℝ}
    (hNS : LoggedPreterminalNavierStokesAdmissible u T)
  ...
  have hBound :
    ...
    have hApprox :=
      norm_sq_selectedOldDifferenceReal_le_energyIntegral_add_epsilonError_of_allProjectedRHSWeakFTC
        hNS ht htau hEnd hE hTail htauR hWeakFTC
        hq hεn
