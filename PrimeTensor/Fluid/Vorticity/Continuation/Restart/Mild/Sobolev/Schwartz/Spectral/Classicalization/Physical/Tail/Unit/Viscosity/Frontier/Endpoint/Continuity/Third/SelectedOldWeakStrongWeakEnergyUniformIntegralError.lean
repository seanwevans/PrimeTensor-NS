import PrimeTensor.Fluid.Vorticity.Continuation.Restart.Mild.Sobolev.Schwartz.Spectral.Classicalization.Physical.Tail.Unit.Viscosity.Frontier.Endpoint.Continuity.Third.SelectedOldWeakStrongWeakEnergyUniformRemainder

/-!
# Uniform-mesh integral approximation error

The remaining mesh-dependent error inside the weak-energy integral is

    2 * Σ_k ∫_{x_k}^{x_{k+1}}
      (ε + ‖D(x_k) - D(r)‖) * 6 C(E) dr.

The uniform temporal modulus proved previously gives, on every cell,

    ‖D(x_k) - D(r)‖ ≤ 6 C(E) q / n.

Hence every integrand is bounded by the corresponding constant, and summing
over the `n` cells gives

    12 ε C(E) q + 72 C(E)^2 q^2 / n.

This closes the final mesh-dependent approximation term.  The only term left
after this checkpoint is the genuine energy integral
`6 * gradientEnvelope(E) * ∫ ‖D(r)‖² dr`.
-/
...
noncomputable local instance axisFintypeH3SelectedOldWeakStrongWeakEnergyUniformIntegralError
    (d : Depth) :
    Fintype (PrimeTensor.Axis d) :=
  Fintype.ofFinite (PrimeTensor.Axis d)

/-- One uniform cell's integral approximation error is bounded by the constant
obtained from the strong selected--old temporal modulus. -/
theorem selectedOldDifference_uniformPartition_cellIntegralError_le
    {E : ℝ}
  ...
  have hPointwise :
    ...
    have hNorm :=
      norm_h3PreterminalSelectedOldDifference_uniformPartition_left_sub_le
    ...
    have hAdd :
      ...
      simpa only [add_comm] using add_le_add_left hNorm ε
...
/-- Summing the within-cell approximation integrals gives an explicit
`O(ε) + O(1/n)` bound. -/
theorem sum_selectedOldDifference_uniformPartition_cellIntegralError_le
    {E : ℝ}
    {u : SpaceTimeVectorField ℝ ℝ MulReal Depth.three}
    {T t tau q ε : ℝ}
    (hNS : LoggedPreterminalNavierStokesAdmissible u T)
  ...
  have hEach :
    ...
    exact
      selectedOldDifference_uniformPartition_cellIntegralError_le
        hNS ht htau hEnd hE hTail htauR hPressure
        hq n k hn hklt hε
