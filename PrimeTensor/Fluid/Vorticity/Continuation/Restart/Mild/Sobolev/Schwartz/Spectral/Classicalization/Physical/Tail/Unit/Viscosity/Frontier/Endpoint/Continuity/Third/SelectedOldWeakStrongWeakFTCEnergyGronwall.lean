import PrimeTensor.Fluid.Vorticity.Continuation.Restart.Mild.Sobolev.Schwartz.Spectral.Classicalization.Physical.Tail.Unit.Viscosity.Frontier.Endpoint.Continuity.Third.SelectedOldWeakStrongWeakFTCEnergyExact
...
/-!
# Grönwall and physical agreement from scalar old weak FTC

The scalar weak-FTC route now gives
...
The scalar-FTC Lipschitz theorem gives continuity of `D`, hence the FTC gives
`H' = ‖D‖²`.  The exact relative-energy inequality becomes the scalar
Grönwall inequality
...
The concrete `L²` representation bridge then upgrades this to pointwise
selected--old physical agreement and decoder agreement.
-/
...
noncomputable local instance axisFintypeH3SelectedOldWeakStrongWeakFTCEnergyGronwall
    (d : Depth) :
    Fintype (PrimeTensor.Axis d) :=
  Fintype.ofFinite (PrimeTensor.Axis d)

/-- The exact scalar-weak-FTC relative-energy inequality forces the ambient
selected--old physical difference to vanish. -/
theorem h3PreterminalSelectedOldVelocityPhysicalL2DifferenceRealOnElapsed_eq_zero_of_allProjectedRHSWeakFTC
    {E : ℝ}
  ...
  have hEnergy :
    ...
    have h :=
      norm_sq_selectedOldDifferenceReal_le_energyIntegral_of_allProjectedRHSWeakFTC
        hNS ht htau hEnd hE hTail htauR hWeakFTC hs
...
/-- Subtype form of scalar-weak-FTC weak-energy uniqueness. -/
theorem h3PreterminalSelectedOldVelocityPhysicalL2DifferenceOnElapsed_eq_zero_of_allProjectedRHSWeakFTC
    {E : ℝ}
    {u : SpaceTimeVectorField ℝ ℝ MulReal Depth.three}
    {T t tau : ℝ}
    (hNS : LoggedPreterminalNavierStokesAdmissible u T)
    ...
    0 := by
  have h :=
    h3PreterminalSelectedOldVelocityPhysicalL2DifferenceRealOnElapsed_eq_zero_of_allProjectedRHSWeakFTC
      hNS ht htau hEnd hE hTail htauR hWeakFTC q.property
...
/-- Scalar-weak-FTC uniqueness upgrades to pointwise selected/old physical
agreement at one positive elapsed time. -/
theorem h3PreterminalSelectedPhysicalAgreementAt_of_allProjectedRHSWeakFTC
    {E : ℝ}
    {u : SpaceTimeVectorField ℝ ℝ MulReal Depth.three}
    {T t tau q : ℝ}
    (hNS : LoggedPreterminalNavierStokesAdmissible u T)
  ...
  have hZero :
      ...
      0 := by
    exact
      h3PreterminalSelectedOldVelocityPhysicalL2DifferenceOnElapsed_eq_zero_of_allProjectedRHSWeakFTC
        hNS ht htau hEnd hE hTail htauR hWeakFTC qClosed
...
/-- Fixed-interval selected/old physical agreement from scalar old weak FTC. -/
theorem h3PreterminalSelectedPhysicalAgreementOnElapsed_of_allProjectedRHSWeakFTC
    {E : ℝ}
    {u : SpaceTimeVectorField ℝ ℝ MulReal Depth.three}
    {T t tau : ℝ}
    (hNS : LoggedPreterminalNavierStokesAdmissible u T)
...
/-- Fixed-interval selected decoder agreement from scalar old weak FTC. -/
theorem h3PreterminalSelectedDecoderAgreementOnElapsed_of_allProjectedRHSWeakFTC
    {E : ℝ}
    {u : SpaceTimeVectorField ℝ ℝ MulReal Depth.three}
    {T t tau : ℝ}
    (hNS : LoggedPreterminalNavierStokesAdmissible u T)
  ...
  exact
    h3PreterminalSelectedDecoderAgreesAt_of_physicalAgreement
      ...
      (q : ℝ)
      hNS ht hE hTail
      (h3PreterminalSelectedPhysicalAgreementOnElapsed_of_allProjectedRHSWeakFTC
        hNS ht htau hEnd hE hTail htauR hWeakFTC q)
