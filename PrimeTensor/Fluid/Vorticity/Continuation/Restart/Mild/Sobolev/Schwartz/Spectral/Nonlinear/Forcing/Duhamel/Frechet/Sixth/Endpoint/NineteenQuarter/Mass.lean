import PrimeTensor.Fluid.Vorticity.Continuation.Restart.Mild.Sobolev.Schwartz.Spectral.Nonlinear.Forcing.Duhamel.Frechet.Sixth.Endpoint.NineteenQuarter.Section
...
/-!
# Sixth Fréchet endpoint: subcritical nineteen-quarter source mass

`NineteenQuarterSection` proves the one-source estimate
...
The terminal singularity is integrable. Therefore any interval-uniform cubic
forcing bound
...
closes product-space integrability and gives the explicit source budget
    ...
    ∬ |ξ|^(19/4) |H_{t-s} N_s| dξ ds
      ≤
    B · 8 C₇(ν) (t-a)^(1/8).

As in the fifth layer, this checkpoint keeps `B` abstract. The next layer can
instantiate it from an interval-uniform selected forcing cubic envelope.
-/
...
noncomputable local instance axisFintypeH3SchwartzSixthEndpointNineteenQuarterMass
    (d : Depth) :
    Fintype (PrimeTensor.Axis d) :=
  Fintype.ofFinite (PrimeTensor.Axis d)

/-- Selected Duhamel source kernel carrying the radial `19/4` Fourier weight. -/
noncomputable def h3SelectedDuhamelNineteenQuarterComplexKernel
    (ν A t : ℝ)
    (hν : 0 < ν)
  ...
  ((h3FourierNineteenQuarterWeight p.2 : ℝ) : ℂ) *
    h3SelectedDuhamelTailComplexKernel
      ν A t hν U₀ hA hU₀ i p

/-- The selected weighted `19/4` source kernel is jointly strongly measurable
on every positive terminal interval. -/
theorem h3SelectedDuhamelNineteenQuarterComplexKernel_aestronglyMeasurable
    {ν A a t : ℝ}
    (hν : 0 < ν)
    (U₀ : H3SpectralVelocityState)
    (hA : 0 < A)
  ...
  have hWeightReal :
      Continuous
        (fun p : ℝ × H3FourierPoint3 =>
          h3FourierNineteenQuarterWeight p.2) := by
  ...
  unfold h3SelectedDuhamelNineteenQuarterComplexKernel
  exact hWeight.mul hSource

/-- Every strict source-time frequency section of the selected `19/4` kernel is
integrable. -/
theorem h3SelectedDuhamelNineteenQuarterComplexKernel_section_integrable
    {ν A a t s : ℝ}
    (hν : 0 < ν)
    (U₀ : H3SpectralVelocityState)
    (hA : 0 < A)
...
normalized `7/4` terminal majorant times the selected forcing cubic mass. -/
theorem h3SelectedDuhamelNineteenQuarterComplexKernel_frequencyNormIntegral_le
    {ν A a t s : ℝ}
    (hν : 0 < ν)
    (U₀ : H3SpectralVelocityState)
    (hA : 0 < A)
  ...
  have hSection :=
    h3SelectedForcingHeat_nineteenQuarter_frequencyIntegral_le
...
/-- Explicit source-time budget for a selected `19/4` Duhamel source whose
forcing cubic mass is uniformly bounded by `B` on `(a,t)`. -/
noncomputable def h3SelectedDuhamelNineteenQuarterSourceBudget
    (ν a t B : ℝ) : ℝ :=
  B *
    (8 *
...
/-- Product-space integrability of the selected `19/4` source kernel under a
uniform forcing cubic-mass bound. -/
theorem h3SelectedDuhamelNineteenQuarterComplexKernel_fubini_integrable_of_thirdMass_le
    {ν A a t B : ℝ}
    (hν : 0 < ν)
    (U₀ : H3SpectralVelocityState)
    (hA : 0 < A)
  ...
  have hJoint :
      AEStronglyMeasurable
        ...
        (((volume : Measure ℝ).restrict (Set.Ioo a t)).prod
          (volume : Measure H3FourierPoint3)) :=
    h3SelectedDuhamelNineteenQuarterComplexKernel_aestronglyMeasurable
  ...
  · filter_upwards [ae_restrict_mem measurableSet_Ioo] with s hs
    exact
      h3SelectedDuhamelNineteenQuarterComplexKernel_section_integrable
        hν U₀ hA hU₀ ha hat htR hs i
  ...
  · have hOuterMeas :
    ...
    have hSection :
      ...
      dsimp only [M, W]
      simpa using
        (h3SelectedDuhamelNineteenQuarterComplexKernel_frequencyNormIntegral_le
          hν U₀ hA hU₀ ha hat htR hs i)
...
/-- Quantitative iterated norm budget for the selected `19/4` source kernel
under a uniform forcing cubic-mass bound. -/
theorem h3SelectedDuhamelNineteenQuarterComplexKernel_iteratedNormIntegral_le_of_thirdMass_le
    {ν A a t B : ℝ}
    (hν : 0 < ν)
    (U₀ : H3SpectralVelocityState)
    (hA : 0 < A)
    ...
    (∫ s : ℝ,
      ...
      ∂μt)
      ≤
    h3SelectedDuhamelNineteenQuarterSourceBudget ν a t B := by
  dsimp only
