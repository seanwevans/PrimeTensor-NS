import PrimeTensor.Fluid.Vorticity.Continuation.Restart.Mild.Sobolev.Schwartz.Spectral.Classicalization.Physical.Tail.Unit.Viscosity.Frontier.Endpoint.Continuity.Third.SelectedOldWeakStrongWeakEnergyUniformIntegralError

/-!
# Collapse the uniform weak-energy partition

All mesh-dependent error terms are now explicitly controlled.  This file
performs the remaining exact bookkeeping.

First, `intervalIntegral.sum_integral_adjacent_intervals` collapses the uniform
cell integrals of the genuine energy density to the single interval `[0,q]`.

Second, the cellwise integral is split into its energy and approximation-error
parts.  Combining the previously proved summed bounds yields the finite-mesh
estimate
...
This is the final finite-partition inequality.  The remaining steps are purely
limiting: first remove `1/n`, then remove `ε`, and apply Grönwall.
-/
...
noncomputable local instance axisFintypeH3SelectedOldWeakStrongWeakEnergyUniformCollapse
    (d : Depth) :
    Fintype (PrimeTensor.Axis d) :=
  Fintype.ofFinite (PrimeTensor.Axis d)

/-- The genuine energy part of the uniform partition telescopes exactly to the
single energy integral on `[0,q]`. -/
theorem sum_selectedOldDifference_uniformPartition_energyIntegral_eq
    {E : ℝ}
...
/-- Final finite-mesh weak-energy inequality, with every approximation error
made explicit. -/
theorem norm_sq_selectedOldDifferenceReal_le_energyIntegral_add_uniformErrors
    {E : ℝ}
    {u : SpaceTimeVectorField ℝ ℝ MulReal Depth.three}
    {T t tau q ε : ℝ}
    (hNS : LoggedPreterminalNavierStokesAdmissible u T)
  ...
  have hPart :=
    norm_sq_selectedOldDifferenceReal_le_uniformPartitionApproximateEnergyIncrement
  ...
  have hEnergy :=
    sum_selectedOldDifference_uniformPartition_energyIntegral_eq
      hNS ht htau hEnd hE hTail hPressure
      hq n hn

  have hError :=
    sum_selectedOldDifference_uniformPartition_cellIntegralError_le
      hNS ht htau hEnd hE hTail htauR hPressure
      hq n hn hε.le

  have hRemainder :=
    sum_selectedOldDifference_uniformPartition_endpointAndQuadratic_le
      hNS ht htau hEnd hE hTail htauR hPressure
      hq n hn hε.le
