...
/-!
# Endpoint-closed interaction derivative

The existing interaction derivative is stated for a final target
`q ∈ (0,tau)`.  Its proof only uses that every source time `s < q` remains
strictly inside the ambient endpoint interval.

Hence the same argument works for `q = tau`: from

    s < q ≤ tau

we still obtain `s < tau`.

This file records that endpoint-closed form without changing the established
...
noncomputable local instance axisFintypeH3PhysicalL2TemporalEndpointFourierL2InteractionDerivativeEndpoint
    (d : Depth) :
    Fintype (PrimeTensor.Axis d) :=
  Fintype.ofFinite (PrimeTensor.Axis d)

/-- On `0 < s < q ≤ tau`, the quotient-safe interaction picture has right
derivative equal to the heat-evolved negative endpoint forcing. -/
theorem h3PreterminalTailCanonicalRawFourierL2InteractionReal_hasDerivWithinAt_right_of_integralEvolution_to
    {E : ℝ}
