...
/-!
# Classicalization: quartic weighted-Fourier time continuity

The quartic interpolation estimate turns the selected path's H³ continuity and
one locally uniform fifth-moment bound into continuity in the quartic weighted
raw-Fourier topology.

The generic theorem below isolates the mechanism.  If a scalar spectral path
`F` is H³-continuous at an interior time `s`, and its fifth raw Fourier moment
is integrable and uniformly bounded on a closed neighborhood `[a,b]` of `s`,
then

    M₄(F(r) - F(s)) → 0  as r → s.

The proof uses the compiled estimate
...
the fifth-moment tail is below `ε/2`; then H³ continuity makes the low
frequency term below `ε/2`.

The selected-path specialization obtains the local fifth-moment bound from the
generic natural moment slab at order five.  Hence every strict interior
positive time of the canonical restart interval has quartic weighted-Fourier
time continuity.
...
noncomputable local instance axisFintypeH3SchwartzClassicalizationQuarticTimeContinuity
    (d : Depth) :
...
uniform fifth raw Fourier moment implies quartic weighted-Fourier continuity. -/
theorem h3SpectralScalarRawFourierMomentMass_four_sub_tendsto_zero_of_continuousAt_of_fifthMoment_bound
    ...
    (hFifthInt :
      ∀ r ∈ Set.Icc a b,
        H3RawFourierMomentIntegrable (5 : ℝ) (F r))
    (hFifthBound :
      ∀ r ∈ Set.Icc a b,
        h3SpectralScalarRawFourierMomentMass (5 : ℝ) (F r) ≤ B) :
    Tendsto
      (fun r : ℝ =>
  ...
  have hsFifthInt :
      H3RawFourierMomentIntegrable (5 : ℝ) (F s) :=
    hFifthInt s hsIcc

  have hsFifthBound :
      h3SpectralScalarRawFourierMomentMass (5 : ℝ) (F s) ≤ B :=
    hFifthBound s hsIcc
  ...
  · intro ε hε
    ...
    let C : ℝ :=
      R ^ 4 * h3RawFourierL1DeweightingCoefficient
    ...
    have hInterp :=
      h3SpectralScalarRawFourierMomentMass_four_sub_le_norm
        (F r) (F s) hR hrFifthInt hsFifthInt
    ...
    have hTail :
        R⁻¹ *
            (h3SpectralScalarRawFourierMomentMass (5 : ℝ) (F r) +
              h3SpectralScalarRawFourierMomentMass (5 : ℝ) (F s))
          < ε / 2 := by
      apply lt_of_le_of_lt _ hTailBudget
      exact
        mul_le_mul_of_nonneg_left
          (add_le_add hrFifthBound hsFifthBound)
          hInv0
...
the quartic weighted raw-Fourier difference topology at each strict positive
interior time of the restart interval. -/
theorem h3SpectralFinHeatLerayMildSolutionAtRestartRadius_quarticDifferenceMass_tendsto_zero
    {ν A s : ℝ}
    (hν : 0 < ν)
  ...
  have hFive : (((5 : ℕ) : ℝ)) = (5 : ℝ) := by
    norm_num

  have hFifthInt :
      ∀ r ∈ Set.Icc a b,
        H3RawFourierMomentIntegrable (5 : ℝ) (W r i) := by
    intro r hr
  ...
  have hFifthBound :
      ∀ r ∈ Set.Icc a b,
        h3SpectralScalarRawFourierMomentMass (5 : ℝ) (W r i) ≤ BState := by
    intro r hr
    have hAt := hData r hr i
    dsimp only at hAt
    rw [hFive] at hAt
  ...
  have hGeneric :=
    h3SpectralScalarRawFourierMomentMass_four_sub_tendsto_zero_of_continuousAt_of_fifthMoment_bound
      (F := fun r : ℝ => W r i)
      (a := a)
      (s := s)
      (b := b)
