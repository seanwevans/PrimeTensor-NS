import PrimeTensor.Fluid.Vorticity.Continuation.Restart.Mild.Sobolev.Schwartz.Spectral.Classicalization.Selected.Velocity.Time.Derivative.Spatial.C3
...
selected real time-derivative `C¹` and `C²` files.
-/
...
noncomputable local instance axisFintypeH3SelectedVelocityRealTimeDerivativeSpatialC3
    (d : Depth) :
...
theorem h3SpectralFinHeatLerayMildSolutionAtRestartRadius_realC1RepresentativeOnPoint3_timeDerivative_spatial_contDiff_three
    {ν A t : ℝ}
    (hν : 0 < ν)
  ...
  have hComplex :
    ...
    dsimp only [gC, W]
    exact
      h3SpectralFinHeatLerayMildSolutionAtRestartRadius_C1Representative_timeDerivative_spatial_contDiff_three
        hν U₀ hA hU₀ ht htR i

  have hToLp :
      ContDiff ℝ 3 toLp := by
    dsimp only [toLp]
...
theorem h3SpectralFinHeatLerayMildSolutionAtRestartRadius_selectedRealVelocity_component_timeDerivative_spatial_contDiff_three
    {ν A t : ℝ}
    (hν : 0 < ν)
    (U₀ : H3SpectralVelocityState)
    (hA : 0 < A)
