import PrimeTensor.Fluid.Vorticity.Continuation.Restart.Mild.Sobolev.Schwartz.Spectral.Classicalization.Physical.Tail.Unit.Viscosity.Frontier.Endpoint.Continuity.Third.SelectedOldWeakStrongWeakFTCEnergyUniformModulus

/-!
# Uniform-mesh integral error from scalar old weak FTC
...
The scalar weak-FTC temporal modulus gives on each uniform cell
...
noncomputable local instance axisFintypeH3SelectedOldWeakStrongWeakFTCEnergyUniformIntegralError
    (d : Depth) :
    Fintype (PrimeTensor.Axis d) :=
  Fintype.ofFinite (PrimeTensor.Axis d)

/-- One uniform cell's integral approximation error from scalar weak FTC. -/
theorem selectedOldDifference_uniformPartition_cellIntegralError_le_of_allProjectedRHSWeakFTC
    {E : ℝ}
  ...
  have hPointwise :
    ...
    have hNorm :=
      norm_h3PreterminalSelectedOldDifference_uniformPartition_left_sub_le_of_allProjectedRHSWeakFTC
...
/-- Summing the scalar-weak-FTC within-cell approximation integrals yields the
explicit `O(ε) + O(1/n)` bound. -/
theorem sum_selectedOldDifference_uniformPartition_cellIntegralError_le_of_allProjectedRHSWeakFTC
    {E : ℝ}
    {u : SpaceTimeVectorField ℝ ℝ MulReal Depth.three}
    {T t tau q ε : ℝ}
    (hNS : LoggedPreterminalNavierStokesAdmissible u T)
  ...
  have hEach :
    ...
    exact
      selectedOldDifference_uniformPartition_cellIntegralError_le_of_allProjectedRHSWeakFTC
        hNS ht htau hEnd hE hTail htauR hWeakFTC
        hq n k hn hklt hε
