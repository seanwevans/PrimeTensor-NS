...
/-!
# Fifth Fréchet endpoint: subcritical fifteen-quarter source sections

A direct full-fourth varying-source estimate is critical: pairing the selected
forcing second moment with two heat powers produces the nonintegrable terminal
kernel `(t-s)⁻¹`.

The correct next bootstrap is therefore slightly subcritical.  At weight
`15/4`,

    15/4 = 2 + 7/4.

The selected forcing supplies the first two powers, while the residual `7/4`
heat multiplier is exactly the already-normalized integrable majorant

    C₇(ν) (t-s)^(-7/8).

This file isolates that frequency-section estimate.  No source subtraction or
terminal cancellation is needed.
-/
...
noncomputable local instance axisFintypeH3SchwartzFifthEndpointFifteenQuarterSection
    (d : Depth) :
    Fintype (PrimeTensor.Axis d) :=
  Fintype.ofFinite (PrimeTensor.Axis d)

/-- The radial `15/4` Fourier weight. -/
noncomputable def h3FourierFifteenQuarterWeight
    (ξ : H3FourierPoint3) : ℝ :=
  ‖ξ‖ ^ ((15 : ℝ) / 4)

/-- The `15/4` weight factors into the selected forcing second moment and the
residual `7/4` heat weight. -/
theorem h3FourierFifteenQuarterWeight_eq_sq_mul_sevenQuarter
    (ξ : H3FourierPoint3) :
    h3FourierFifteenQuarterWeight ξ
      =
    ‖ξ‖ ^ 2 * h3FourierSevenQuarterWeight ξ := by
  have hξ0 : 0 ≤ ‖ξ‖ :=
    norm_nonneg ξ
...
/-- A positive heat lag sends an amplitude with an integrable second raw
Fourier moment to an amplitude with an integrable `15/4` moment. -/
theorem h3HeatFourierSymbol_fifteenQuarter_weighted_mul_integrable_of_second
    {ν τ : ℝ}
    (hν : 0 < ν)
    (hτ : 0 < τ)
    (F : H3FourierPoint3 → ℂ)
  ...
  rw [
    ...
    norm_mul,
    h3FourierFifteenQuarterWeight_eq_sq_mul_sevenQuarter
  ]

  calc
    (‖ξ‖ ^ 2 * h3FourierSevenQuarterWeight ξ) *
        (‖h3HeatFourierSymbol ν τ ξ‖ * ‖F ξ‖)
        =
      ‖ξ‖ ^ 2 *
    ...
    _ ≤
      ...
      have hScaled :
          ‖ξ‖ ^ 2 *
            ...
            ≤
          ‖ξ‖ ^ 2 * C :=
        mul_le_mul_of_nonneg_left hHeat hSq0
      exact
...
/-- Quantitative one-section estimate at the `15/4` endpoint. -/
theorem h3HeatFourierSymbol_fifteenQuarter_frequencyIntegral_le
    {ν t s : ℝ}
    (hν : 0 < ν)
    (hs : s < t)
    (F : H3FourierPoint3 → ℂ)
...
subcritical `15/4` state endpoint. -/
theorem h3SelectedForcingHeat_fifteenQuarter_frequencyIntegral_le
    {ν A a t s : ℝ}
    (hν : 0 < ν)
  ...
  have hBase :=
    h3HeatFourierSymbol_fifteenQuarter_frequencyIntegral_le
      hν hs.2 N hN hN2
