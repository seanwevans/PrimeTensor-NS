...
/-!
# Positive-time derivative of the scalar heat third coordinate

The ordered third spatial heat coordinate reconstructs

    D_a(ξ) D_b(ξ) D_c(ξ) heatRaw(ν,t,G,ξ).

Differentiating in heat time replaces `heatRaw` by its time generator.  The
three coordinate symbols cost three Fourier powers and the heat generator
costs two more, so the already-compiled positive-time fifth heat moment gives
a local dominated majorant.
...
noncomputable local instance axisFintypeH3SchwartzHeatTimeThirdCoordinateTimeDerivative
    (d : Depth) :
...
/-- Raw time-generator amplitude for one ordered third heat coordinate. -/
noncomputable def h3SpectralScalarHeatThirdCoordinateTimeGeneratorRawAmplitude
    (ν t : ℝ)
...
/-- The third-coordinate heat time-generator raw amplitude is strongly
measurable. -/
theorem h3SpectralScalarHeatThirdCoordinateTimeGeneratorRawAmplitude_aestronglyMeasurable
    (ν t : ℝ)
    (G : H3SpectralScalarState)
    (a b c : Fin 3) :
...
/-- Physical inverse-Fourier representative of the third-coordinate heat time
generator. -/
noncomputable def h3SpectralScalarHeatThirdCoordinateTimeGeneratorRepresentative
    (ν t : ℝ)
    (G : H3SpectralScalarState)
    (a b c : Fin 3) :
    H3FourierPoint3 → ℂ :=
...
/-- The positive-time ordered third heat coordinate has an ordinary time
derivative equal to its named third-coordinate heat generator. -/
theorem h3SpectralScalarHeatThirdCoordinateRepresentative_hasDerivAt_time
    {ν t : ℝ}
    (hν : 0 < ν)
    (ht : 0 < t)
    (G : H3SpectralScalarState)
  ...
  have hF'_meas :
    ...
    exact
      hPhaseContinuous.aestronglyMeasurable.mul
        (h3SpectralScalarHeatThirdCoordinateTimeGeneratorRawAmplitude_aestronglyMeasurable
          ν t G a b c)
  ...
  have h_bound :
    ...
    have hDc' :
        ‖h3FourierDerivativeSymbol c ξ‖
          ≤
        (2 * Real.pi) * ‖ξ‖ := by
      simpa only [h3FourierGradientMagnitude] using hDc
