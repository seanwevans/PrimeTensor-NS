...
* `q > 0`: since always `q ≤ tau`, the endpoint-safe vorticity H³
  variation-of-constants theorem applies, including at `q = tau`.

Thus endpoint physical `L²` continuity is now the only local hypothesis left.
-/
...
noncomputable local instance axisFintypeH3PhysicalTailEndpointCanonicalMildVorticity
    (d : Depth) :
    Fintype (PrimeTensor.Axis d) :=
  Fintype.ofFinite (PrimeTensor.Axis d)

/-- The complete unit-viscosity restarted mild identity for the endpoint-only
canonical preterminal path follows from endpoint physical `L²` continuity
alone. -/
theorem h3PreterminalTailCanonicalSpectralPhysicalPathOfL2Endpoint_mild_endpointH3
    {E : ℝ}
  ...
  · have hqPos : 0 < q :=
    ...
    have hVoC :=
      h3PreterminalTailCanonicalNormalizedRealPathOfL2Endpoint_variationOfConstants_endpointH3_to
        hNS ht htau hEnd hE hTail hEndpoint hqTo
...
/-- Endpoint physical `L²` continuity alone now supplies the complete local
unit-viscosity physical-evolution package. -/
theorem h3PreterminalTailPhysicalEvolutionAt_of_endpointH3
    {E : ℝ}
    {u : SpaceTimeVectorField ℝ ℝ MulReal Depth.three}
    {T t tau : ℝ}
    (hNS : LoggedPreterminalNavierStokesAdmissible u T)
  ...
  exact
    h3PreterminalTailCanonicalSpectralPhysicalPathOfL2Endpoint_mild_endpointH3
      hNS
      ht
      htau
      hEnd
