...
import PrimeTensor.Fluid.Vorticity.Continuation.Restart.Mild.Sobolev.Schwartz.Spectral.Classicalization.Physical.Tail.Endpoint.Canonical.Physical.L2.Admissible.Closure.Leray.Density.Temporal.Endpoint.Fourier.L2.Duhamel.Identification.WeakDerivative.Endpoint

/-!
# Endpoint canonical restarted mild equation from scalar weak derivatives

The scalar weak-pairing derivative route now reaches the actual weighted H³
...
* `q > 0`: since always `q ≤ tau`, the endpoint-safe scalar weak-derivative H³
  variation-of-constants theorem applies, including at `q = tau`.

Thus neither coordinatewise product-space integrability nor Fubini appears in
the mild-evolution hypothesis.
-/
...
noncomputable local instance axisFintypeH3PhysicalTailEndpointCanonicalMildWeakDerivative
    (d : Depth) :
    Fintype (PrimeTensor.Axis d) :=
  Fintype.ofFinite (PrimeTensor.Axis d)

/-- The complete unit-viscosity restarted mild identity for the endpoint-only
canonical preterminal path follows from the all-tests scalar weak-pairing
derivative frontier. -/
theorem h3PreterminalTailCanonicalSpectralPhysicalPathOfL2Endpoint_mild_of_all_weakPairingDerivatives
    {E : ℝ}
  ...
  · have hqPos : 0 < q :=
    ...
    have hVoC :=
      h3PreterminalTailCanonicalNormalizedRealPathOfL2Endpoint_variationOfConstants_of_all_weakPairingDerivatives_to
...
`L²` continuity plus the scalar weak-pairing derivative frontier. -/
theorem h3PreterminalTailPhysicalEvolutionAt_of_endpoint_all_weakPairingDerivatives
    {E : ℝ}
    {u : SpaceTimeVectorField ℝ ℝ MulReal Depth.three}
    {T t tau : ℝ}
    (hNS : LoggedPreterminalNavierStokesAdmissible u T)
  ...
  exact
    h3PreterminalTailCanonicalSpectralPhysicalPathOfL2Endpoint_mild_of_all_weakPairingDerivatives
      hNS
      ht
      htau
      hEnd
