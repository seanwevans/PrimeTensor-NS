import PrimeTensor.Fluid.Vorticity.Continuation.Landau.H3PathOrderThreeSelectedVelocityFifthRadialL2Difference

/-!
# Selected fifth-radial velocity Fourier L² continuity

The previous checkpoint reduced fifth-radial selected velocity differences on a
positive compact restart slab to two quantities already under control:

* strong continuity of the unweighted selected raw Fourier `L²` state;
* one slab-uniform sixth-radial selected velocity norm-square bound.
...
Choosing `R` first makes the sixth-order tail uniformly small; with that fixed,
raw Fourier `L²` continuity controls the low-frequency part.  We first prove
continuity on a positive compact slab and then promote it to the whole strict
restart interval by `Set.IccExtend`.
-/
...
noncomputable local instance axisFintypeH3PathOrderThreeSelectedVelocityFifthRadialL2Continuity
    (d : Depth) :
...
/-- The selected fifth-radial velocity state is strongly continuous on every
positive compact restart slab. -/
theorem continuous_h3PreterminalSelectedVelocityFifthRadialFourierL2OnCompact
    {E : ℝ}
  ...
  obtain ⟨B, hB0, hB⟩ :=
    exists_norm_sq_bound_h3PreterminalSelectedVelocitySixthRadialFourierL2OnCompact
      (one_pos : (0 : ℝ) < 1)
      U₀ hA hU₀
      ha hab hbR
  ...
  have hSqTend :
    ...
    · intro ε hε
      ...
      have hInterp :=
        norm_sq_h3PreterminalSelectedVelocityFifthRadialFourierL2_sub_leOnCompact
      ...
      have hSqLt :
        ...
        have hInterp' :
            ...
            4 * (R⁻¹) ^ 2 *
              (‖F6 s‖ ^ 2 +
               ‖F6 s₀‖ ^ 2) := by
          dsimp only [F5, F6, F0, toOpen, W, U₀, hA, hU₀]
          exact hInterp
...
whose upper endpoint remains below the restart radius. -/
private theorem exists_h3SelectedVelocityFifthRadialLocalCompactSlab
    {R q : ℝ}
    (hq0 : 0 < q)
    (hqR : q < R) :
    ∃ a b : ℝ,
...
/-- Fifth-radial selected velocity Fourier `L²` is strongly continuous on the
entire strict restart interval. -/
theorem continuous_h3PreterminalSelectedVelocityFifthRadialFourierL2OnRestartRadius
    {E : ℝ}
    {u : SpaceTimeVectorField ℝ ℝ MulReal Depth.three}
    {T t₀ : ℝ}
    (hNS : LoggedPreterminalNavierStokesAdmissible u T)
  ...
  obtain ⟨a, b, ha, hab, hbR, haq₀, hq₀b⟩ :=
    exists_h3SelectedVelocityFifthRadialLocalCompactSlab
  ...
  have hFslab :
    ...
    dsimp only [Fslab]
    exact
      continuous_h3PreterminalSelectedVelocityFifthRadialFourierL2OnCompact
        hNS ht₀ hE hTail
        ha hab hbR j
