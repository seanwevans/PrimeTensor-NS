import PrimeTensor.Fluid.Vorticity.Continuation.Landau.H3PathSelectedPressurePhysicalL2First
import PrimeTensor.Fluid.Vorticity.Continuation.Landau.H3PathPressureL2OrderZero

/-!
# Close order-one pressure L² on the old H³ path

The selected canonical pressure Hessian is now known to belong to physical
`L²`.  The zero-order pressure closure already established equality of the
selected, old chosen, and canonical split pressure gradients on the local
restart overlap.

Because those are equalities of scalar fields, applying
`congrArg (spatial3.d i)` transfers them one spatial derivative higher without
any new analytic estimate.  Thus every `momentumPressure1Component` of the
canonical split pressure belongs to physical `L²` at every strict H³
energy-class time.
-/
...
noncomputable local instance axisFintypeH3PathPressureL2OrderOne
    (d : Depth) :
    Fintype (PrimeTensor.Axis d) :=
  Fintype.ofFinite (PrimeTensor.Axis d)

noncomputable local instance point3MeasureSpaceH3PathPressureL2OrderOne :
    MeasureSpace Point3 :=
...
/-- Every order-one pressure component of the canonical split pressure belongs
 to physical `L²` at every strict H³ energy-class time. -/
theorem h3PathEnergyClassProducesPressure1MemLp2_closed :
    ∀
  ...
  intro u T hH3 a hClass t ht i j
  ...
  have hSelectedRel :
      MemLp
        ...
        (volume : Measure Point3) := by
    have h :=
      h3SelectedRestartPressureSpatialDerivative_two_memLp2
        (t := q)
        (one_pos : (0 : ℝ) < 1)
        U₀ hA hU₀ hq0 hqR i j
    simpa only [
  ...
  have hRelAbs :
      ...
      spatial3.d i
          (spatial3.d j
            (pAbs t)) :=
    congrArg (spatial3.d i) hRelAbs0
  ...
  have hChosenSelected :
      ...
      spatial3.d i
          (spatial3.d j (pAbs t)) :=
    congrArg (spatial3.d i) hChosenSelected0
  ...
  have hSplitChosen :
      ...
      spatial3.d i
          (spatial3.d j (pChosen t)) :=
    congrArg (spatial3.d i) hSplitChosen0
